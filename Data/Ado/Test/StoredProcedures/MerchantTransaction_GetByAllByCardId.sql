-------------------------------------------------------------------------------
-- This will look for merchant transactions by the card identifier.

-------------------------------------------------------------------------------
CREATE PROCEDURE [core].[MerchantTransaction_GetAllByCardId]
(
    @CardId INT
)
AS
BEGIN

    SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;

    SET NOCOUNT ON;

    SELECT
        e.[RowId]            as 'RowId'           ,
        e.[RowVersion]       as 'RowVersion'      ,
        e.[IsActive]         as 'IsActive'        ,
        e.[Flags]            as 'Flags'           ,
        e.[DateCreated]      as 'DateCreated'     ,
        e.[DateModified]     as 'DateModified'    ,
        e.[TransactionGroup] as 'TransactionGroup',
        e.[TransactionType]  as 'TransactionType' ,
        e.[CurrencyCode]     as 'CurrencyCode'    ,
        e.[Amount]           as 'Amount'          ,
        e.[OrderNumber]      as 'OrderNumber'     ,
        e.[CardId]           as 'CardId'
    FROM [core].[MerchantTransactions] e
    WHERE (e.[CardId] = @CardId) AND (e.[IsActive] = 1)

END
