...
namespace Codev.Core.Service.Diagnostic
{
    ...
    public sealed partial class DiagnosticService : BaseService, IDiagnosticService
    {
        ...
        {
            List<ErrorLogEntity> entities = this.ErrorLogRepository.GetAllByDateRange(identityEntity, dateStart, dateEnd).ToList();

            entities.ForEach(x => x.Identity = identityEntity);
        ...
        public List<ErrorLog> GetStatistics(
            ...
            Nullable<LocalDate> dateEnd)
        {
            List<ErrorLogEntity> entities = this.ErrorLogRepository.GetStatistics(identityEntity, dateStart, dateEnd).ToList();
